...
- ปุ่มลอย**มุมซ้ายล่าง** ทุกหน้า portal (`FeedbackFab` mount ครั้งเดียวใน `PortalFrameInner`, z-index 1450 เหนือ drawer/dialog — กดได้ขณะเปิด dialog/drawer, ใต้ tooltip/toast) → แคปหน้าจอด้วย `html-to-image` (dynamic import, ตัด element ที่มี `data-feedback-ignore` = ปุ่ม/toast, ย่อ ≤ 1600px JPEG ≤ 2.5 MB — `src/components/feedback/capture.ts`) → `FeedbackDialog`: ประเภท bug|suggestion, ชื่อผู้แจ้ง (prefill), ความสำคัญ low|medium|high|urgent, รายละเอียด 5–2000, preview + ติ๊กแนบรูป, เมนู (จาก `menuItemForPath`) + path
