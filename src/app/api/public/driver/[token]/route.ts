...
export async function GET(_req: Request, ctx: { params: Promise<{ token: string }> }) {
  try {
    ...
    const [{ data: booking }, { data: shop }, settings] = await Promise.all([
      admin.from('bookings').select(PUBLIC_BOOKING_SELECT).eq('id', ref.id).eq('shop_id', ref.shop_id).maybeSingle(),
    ...
    const { driver_token_version: _v, ...pub } = booking as unknown as Record<string, unknown>;
    void _v;
